...
function Parent({ ParentFunc, ParentText }) {
  ...
  return (
    <div>
      ...
      <div>
        <button type='button' onClick={() => ParentFunc(childData)} >我是子组件按钮</button>
      </div>
    </div>
  )
}
function App() {
  ...
  return (
    <div className="App">
      ...
      {/* 子传父 */}
      <Parent ParentText={ParText} ParentFunc={(data) => {
        setParText(data)
      ...
      {/* Redux Toolkit异步获取数据 */}
      <ul>
        {channelList.map((item, index) => <li key={index}>{item.title}</li>)}
      </ul>
    </div>
  );
}
